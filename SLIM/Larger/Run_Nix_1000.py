...
def parse_slim(slim_string):
    ...
    for line in lines:
        ...
        if line.startswith("OUT:: "):  # Mosquito output
            ...
            this_female = spaced_line[3]
            this_wtM = spaced_line[4]
            this_drM = spaced_line[5]
            this_conversion = spaced_line[6]

            gen.append(this_gen)
            total.append(this_total)
            female.append(this_female)
            wtM.append(this_wtM)
            drM.append(this_drM)
            conversion.append(this_conversion)

    return gen, total, female, wtM, drM, conversion
...
# Get args from arg parser:
    ...
    parser.add_argument('-src', '--source', default="Discrete_Homing_Nix_1000.slim", type=str,
    help=r"SLiM file to be run.")
    parser.add_argument('-outfile', '--output_file', default="Default.xlsx", type=str)
...
def writeOutput(file, data):
    ...
    # 定义表头
    headers = ["Generation", "Total", "Female", "WT Males", "Red Males", "Drive Conversion"]
